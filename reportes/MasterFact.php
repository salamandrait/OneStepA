...
class PDF_Invoice extends FPDF
{
	...
	function addSocieteP($nom,$adresse,$logo)
	{
		...
		$y1 = 12;
		$y2  = $y1 + 10;
		$this->RoundedRect(10, 10, 137, $y2, 1, 'D');
		$this->SetLineWidth(0.3);
		//Positionnement en bas
		//$this->Image($logo, 11 ,11, 24 , 24 ,"");
		$this->SetXY( $x1, $y1 );
	...
	function addSociete($nom,$adresse,$logo,$ext_logo )
	{
		...
		$y1 = 10;
		$y2  = $y1 + 16;
		$this->RoundedRect(10, 6, 137, $y2, 2, 'D');	
		//Positionnement en bas
		$this->Image($logo , 11 ,7, 24 , 24 , $ext_logo);
		$this->SetXY( $x1, $y1 );
		$this->SetFont('Arial','B',9);
		$length = $this->GetStringWidth($nom );
		$this->Cell( $length, 3, $nom);
		$this->SetXY( $x1, $y1 + 5 );
		$this->SetFont('Arial','',9);
		$length = $this->GetStringWidth( $adresse );
		//Coordonnes de la socit
	...
	function fact_dev( $libelle, $num )
	{
		...
		$this->SetFillColor(173,168,170);
		$this->RoundedRect($r1, $y1, ($r2 - $r1), $y2, 1, 'DF');
		$this->SetXY( $r1+10, $y1+2);
		$this->Cell($r2-$r1 -7,4, $texte, 0, 0, "L" );
	}
	...
	function addDate( $item1,$item2 )
	{
		...
		$this->Line( $r1,25,$r2,25);
		$this->SetXY( $r1 + ($r2-$r1)/2 +2, $y1+2 );
		$this->SetFont( "Arial", "B", 11);
		$this->Cell(8,3,$item1, 0,0, "C");

		$this->SetXY( $r1 + ($r2-$r1)/2-15, $y1+2 );
		$this->SetFont( "Arial", "B", 10);
		$this->Cell(8,3,"Tipo:", 0,0, "C");

		$this->SetXY( $r1 + ($r2-$r1)/2+1, $y1+9 );
		$this->SetFont( "Arial", "", 10);
		$this->Cell(8,3,$item2, 0,0, "C");

		$this->SetXY( $r1 + ($r2-$r1)/2-15, $y1+9 );
		$this->SetFont( "Arial", "B", 10);
		$this->Cell(8,3,"Fecha :", 0,0, "C");
	}
	...
	// Client address
	function addClientAdresse( $item1,$item2,$item3,$item4,$item5 )
	{
		...
		$this->SetFont( "Arial", "", 9);
		$this->MultiCell( 190, 4, $item1,0);
		$this->SetXY( $r1+2, $y1+10);
		$this->MultiCell( 30, 4, $item2,0);
		$this->SetXY( $r1+2, $y1+17);
		$this->MultiCell( 190, 4, $item3,0);
		$this->SetXY( $r1+2, $y1+24);
		$this->MultiCell( 100, 4, $item4,0);
		$this->SetXY( 130, $y1+24);
		$this->MultiCell( 190, 4, $item5,0);	
	}

	// Client address
	function addClientAdresseRpt($item1,$item2,$item3,$item4,$item5,$heigh)
	{
			...
			$y2  = $y1-18;
			$this->SetFillColor(255,255,255);
			$this->RoundedRect($r1, $y1, 190, $heigh, 1, 'DF');
			$this->SetXY( $r1, $y1);
			$this->SetFont( "Arial", "B", 9);
			$this->MultiCell( 60, 20, "");
			$this->SetXY( $r1+2, $y1+3);
			$this->SetFont( "Arial", "", 9);
			$this->MultiCell( 190, 3, $item1, 0);
			$this->SetXY( $r1+2, $y1+8);
			$this->MultiCell( 190, 3, $item2, 0);
			$this->SetXY( $r1+2, $y1+12);
			$this->MultiCell(190, 4, $item3, 0);
			$this->SetXY( $r1+2, $y1+24);
			$this->MultiCell( 90, 4, $item4, 0);
			$this->SetXY(170, $y1+2,9,9);
			$this->MultiCell(150, 4, $item5);	
	}
	...
	*/
	...
	function addTVAsRpt($item1,$item2,$item3)
	{
		$this->SetFont('Arial','',9);
		$re  = $this->w - 30;
		$rf  = $this->w - 30;
		...
		$y1  = $this->h - 48;
		$y2  = $y1+10;
		$this->RoundedRect(125, $y1, ($r2 - $r1), ($y2-$y1), 1, 'D');
		$this->Line( $r1+20,  $y1, $r1+20, $y2);
		
		$this->SetFont( "Arial", "B", 10);
		$this->SetXY( $re, $y1-1);
		$this->Cell( 17,4, $item1, '0', '0', 'R');

		$this->SetXY( $re, $y1+4);
		$this->Cell(-25,4, "TOTAL Bs.:", 0, 0, "R");

		$this->SetXY( $r1, $y1-5 );
		$this->Cell( 17,4, $item2, '0', '0', 'R');

		$this->SetXY( $re, $y1+4 );
		$this->Cell( 19,4, $item3, '0', '0', 'R');	
	}

	function addCadre($impuesto)
	{
		$r1  = $this->w - 75;
		$r2  = $r1 + 75;
		$y1  = $this->h - 45;
